...
msg -ama  " $(fun_trans "CONFIGURACION DE SQUID")"
msg -bar
fun_ip
msg -ne " $(fun_trans "Confirme su ip")"; read -p ": " -e -i $IP ip
msg -ama " $(fun_trans "AHORA ELIGE LOS PUERTOS QUE QUIERAS EN SQUID")"
msg -ama " $(fun_trans "Elija los puertos en el ejemplo del orden secuencial 80 8080 8799 3128")"
msg -ne " $(fun_trans "Ingrese los puertos:")"; read portasx
totalporta=($portasx)
unset PORT
   for portx in $(echo $portasx); do
        [[ $(mportas|grep "${portx}") = "" ]] && {
        msg -ama " $(fun_trans "Puerto Elegido:")\033[1;32m ${portx} OK"
        PORT+="${portx}\n"
        } || {
        msg -ama " $(fun_trans "Puerto Elegido:")\033[1;31m ${portx} FAIL"
        }
   done
  [[ -z $PORT ]] && {
  msg -verm " $(fun_trans "Ningun Puerto Valido Fue Elegido")\033[0m"
  return 1
  }
msg -bar
...
msg -ama " $(fun_trans "Ahora elige un Conf para tu Proxy")"
msg -bar
msg -ama  "|1| $(fun_trans "Comun")"
msg -ama  "|2| $(fun_trans "Personalizado") -\033[1;31m $(fun_trans "El usuario debe ajustar")\033[1;37m"
msg -bar
read -p "[1/2]: " -e -i 1 proxy_opt
tput cuu1 && tput dl1
if [[ $proxy_opt = 1 ]]; then
msg -ama  " $(fun_trans "INSTALANDO SQUID COMUN")"
elif [[ $proxy_opt = 1 ]]; then
msg -ama " $(fun_trans "INSTALANDO SQUID PERZONALIZADO")"
else
msg -ama " $(fun_trans "INSTALANDO SQUID COMUN")"
proxy_opt=1
fi
...
tput cuu1 && tput dl1
[[ $hos = \.* ]] && continue
msg -ama " $(fun_trans "Comiense Con") .${cor[0]}"
sleep 2s
tput cuu1 && tput dl1
done
host="$hos/"
...
msg -ama "$(fun_trans "Host Adicionado Con Exito")"
msg -bar
cat $payload | awk -F "/" '{print $1,$2,$3,$4}'
msg -bar
...
echo -e "${cor[4]} $(fun_trans "Hosts Atuales Dentro del Squid")"
msg -bar 
cat $payload | awk -F "/" '{print $1,$2,$3,$4}'
msg -bar
while [[ $hos != \.* ]]; do
...
msg -ama " $(fun_trans "Host Removida Con Exito")${cor[0]}"
msg -bar
cat $payload | awk -F "/" '{print $1,$2,$3,$4}'
msg -bar
if [[ ! -f "/etc/init.d/squid" ]]; then
