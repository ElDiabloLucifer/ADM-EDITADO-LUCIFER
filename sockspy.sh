...
screen -dmS getpy python ${SCPinst}/PGet.py -b "0.0.0.0:$1" -p "${SCPinst}/pwd.pwd"
 ...
 msg -bar
 echo -ne "$(fun_trans "Su Contraseña De Gettunel es"):"
...
msg -ne " $(fun_trans "Digite Una Opcion"): " && read portproxy
tput cuu1 && tput dl1
done
 case $portproxy in
    7)remove_fun && return;;
...
msg -ama "$(fun_trans "Seleccione el puerto en la que los socks se van a ejecutar")"
msg -bar
porta_socket=
...
msg -bar
msg -ne " $(fun_trans "Digite El Texto de Status"): " && read texto_soket
    case $portproxy in
